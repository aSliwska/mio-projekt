...
from Dataset import TicTacToaDataset
...
rice = fetch_ucirepo(id=101)  # not rice
...
train_dataset = TicTacToaDataset(X_train, y_train)
valid_dataset = TicTacToaDataset(X_val, y_val)
test_dataset = TicTacToaDataset(X_test, y_test)
...
if __name__=='__main__':
    ...
    model = MLP(in_channels=9, hidden_channels=[70, 70, 70, 70, 70, 70, 70, 70, 70, 70], activation_layer=nn.ReLU)

    criterion = nn.CrossEntropyLoss()
    optimizer = torch.optim.Adam(model.parameters())
    early_stopping = EarlyStopper(patience=100, min_delta=0.00001)
